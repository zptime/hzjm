...
from upload_resumable.err_code import *
from upload_resumable.models import *
from upload_resumable.utils_data import *
...
@transaction.atomic
def upload_complete(user, file_name, src_file_name, file_size, md5sum=""):
    ...
    if not file_obj:
        file_obj = FileObj.objects.create(name=src_file_name, url=url, size=file_size, type=file_type,
                                          ext=ext, uploader_id=user.id if user.id else 0, md5sum=md5sum)
