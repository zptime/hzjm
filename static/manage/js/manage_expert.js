$(function(){
    new Vue({
        ...
        ready: function () {
			var self = this;
			$('.main_left li#expert').prepend('<el class="li-active-tag"></el>');
			$('.main_left li#expert').children('a').addClass('active');
			self.getTableData();
		...
		methods: {
			getTableData: function() {
				...
				$("#grid").jqGrid({
					...
					autowidth: true,
					height: 'auto',
					colNames:['expert_id','姓名','节目数量','专家介绍','排序','操作'],
					colModel:[
						{name:'expert_id',index:'expert_id',key:true,hidden:true},
						{name:'expert_name',index:'expert_name',align:'center'},
						{name:'expert_article_num',index:'expert_article_num',  align:"center"},
						{name:'expert_intro',index:'expert_intro', width:200, align:"center",hidden:true},
						{name:'expert_sortorder',index:'expert_sortorder', align:"center"},
						{name:'oper',index:'oper', formatter: fmt_oper, align:"center"}
					],
					rowNum : 20,
					pager : '#pager',
					altRows: true,
				...
				function fmt_oper(cellvalue, options, rowObject){
					return '<div class="grid-btn"><a href="/page/manage/expert/edit?id='+rowObject.expert_id+'">修改</a></div>';
				}
			},
			...
			articleAdd: function() {
				window.location.href = "/page/manage/expert/edit";
			},
			articleDelete: function() {
				...
				layer.confirm('确定删除所选文章吗？', {
				    ...
				    self.sendRequest("/api/manage/expert/delete");
				});
			},
